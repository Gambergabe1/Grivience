...
implements Listener {
    ...
    public void onTreeBreak(BlockBreakEvent event) {
        ...
        BukkitTask particleTask = Bukkit.getScheduler().runTaskTimer((Plugin)this.plugin, () -> {
            for (BlockSnapshot snapshot : snapshots) {
                if (!Tag.LOGS.isTagged((Keyed)snapshot.data().getMaterial())) continue;
    ...
    public void onFarmHubProtectedBreak(BlockBreakEvent event) {
        ...
        if (this.isFarmableBreakType(event.getBlock().getType())) {
            return;
        }
        event.setCancelled(true);
        this.warnFarmHubProtection(event.getPlayer(), this.areaDisplayName(area) + " is protected. Only crops and farmable blocks can be broken.");
    }
    ...
    private boolean isFarmableBreakType(Material type) {
        ...
        return Tag.LOGS.isTagged((Keyed)type) || Tag.LEAVES.isTagged((Keyed)type) || Tag.SAPLINGS.isTagged((Keyed)type);
    }
    ...
    private boolean isFarmablePlaceType(Material type) {
        ...
        return Tag.SAPLINGS.isTagged((Keyed)type);
    }
    ...
    private List<BlockSnapshot> treeSnapshotsFor(Block origin, ManagedAreaProfile area) {
        ...
        for (int index = 0; index < queue.size() && snapshots.size() < maxTreeBlocks; ++index) {
            ...
            if (Tag.LEAVES.isTagged((Keyed)current.getType()) || Tag.SAPLINGS.isTagged((Keyed)current.getType())) {
