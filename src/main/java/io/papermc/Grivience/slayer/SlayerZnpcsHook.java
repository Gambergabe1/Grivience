...
public final class SlayerZnpcsHook {
    ...
    private final SlayerGui slayerGui;
    ...
    public SlayerZnpcsHook(GriviencePlugin plugin, SlayerGui slayerGui) {
        this.plugin = plugin;
        this.slayerGui = slayerGui;
    }
    ...
    public void hookIfAvailable() {
        ...
        try {
            ...
            hooked = true;
            plugin.getLogger().info("Hooked Slayer into ZNPCsPlus.");
        } catch (ReflectiveOperationException exception) {
            plugin.getLogger().warning("Slayer ZNPCS hook unavailable: " + exception.getMessage());
        }
    }
    ...
    private void handleNpcInteract(Event event) {
        ...
        try {
            ...
            if (npcId.equalsIgnoreCase("slayer_npc")) {
                Bukkit.getScheduler().runTask(plugin, () -> {
                    if (player.isOnline()) {
                        slayerGui.open(player);
                    }
                });
            }
        } catch (ReflectiveOperationException exception) {
            if (!warnedAboutRuntimeError) {
                ...
                plugin.getLogger().warning("ZNPCS runtime hook error in Slayer: " + exception.getMessage());
            }
        }
    }
}
